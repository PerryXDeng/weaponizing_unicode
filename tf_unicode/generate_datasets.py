...
def compile_datasets(dataset_size):
    ...
    anchors = np.empty((dataset_size,28,28), dtype=np.uint8)
    positives = np.empty((dataset_size,28,28), dtype=np.uint8)
    negatives = np.empty((dataset_size,28,28), dtype=np.uint8)
    for i in range(dataset_size):
        ...
        anchors[i] = draw_char.transformImg(draw_char.drawChar(28, chr(anchor_char), FONT_SIZE, anchor_font))
        positives[i] = draw_char.transformImg(draw_char.drawChar(28, chr(anchor_char), FONT_SIZE, positive_font))
        negatives[i] = draw_char.transformImg(draw_char.drawChar(28, chr(negative_char), FONT_SIZE, negative_font))
    return anchors, positives, negatives


if __name__ == '__main__':
    anchors, positives, negatives = compile_datasets(3)
